<template>
  <div>
    <v-container>
      <v-row>
        <v-col>
          <div class="text-h4 font-weight-bold">メニューリスト</div>
          <div class="text-body-1 text-grey-600 mt-1">全件表示 ({{ displayedMenu.length }}件)</div>
        </v-col>
        <v-col cols="auto">
          ...
          <v-menu :close-on-content-click="false">
            <template #activator="{ props }">
              <v-icon
                ...
                size="small"
                @click="editGenreTags(item.genreIds)"
              />
            </template>
            <v-card width="230px">
              ...
              <v-card-text class="d-flex justify-center">
                <v-chip-group v-model="selectedGenreChips" column multiple mandatory="force">
                  <v-chip v-for="genre in GENRES" :key="genre.id" variant="outlined" filter>
                    {{ genre.name }}
                  </v-chip>
                </v-chip-group>
              </v-card-text>
              <v-card-actions>
                <v-btn block variant="flat" color="primary" @click="updateMenuGenre(item.id)">
                  適用
                </v-btn>
              </v-card-actions>
            </v-card>
          ...
          <v-menu :close-on-content-click="false">
            <template #activator="{ props }">
              <v-icon
                ...
                icon="mdi-cog-outline"
                size="small"
                @click="editCategoryTags(item.categoryIds)"
              />
            </template>
            <v-card width="250px">
              <v-card-title class="text-h6 font-weight-bold">
              ...
              <v-card-actions>
                <v-btn block variant="flat" color="primary" @click="updateMenuCategory(item.id)">
                  適用
                </v-btn>
              </v-card-actions>
            </v-card>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="editedItem.menuName" clearable label="メニュー" />
          ...
          <v-chip-group v-model="selectedCategoryChips" column multiple mandatory="force">
            <v-chip v-for="category in CATEGORIES" :key="category.id" variant="outlined" filter>
              {{ category.name }}
            </v-chip>
          </v-chip-group>
        </v-card-text>
...
const dialogLabel = computed(() =>
  editedItem.value.menuId === 0 ? 'メニュー追加' : 'メニュー編集'
)
